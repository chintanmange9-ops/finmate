...
export function generateAnalytics(transactions: Transaction[], salary: number) {
  const engine = new AnalyticsEngine(transactions);
  ...
  const totalExpenses = transactions
    .filter(t => t.type === 'expense')
    .reduce((sum, t) => sum + t.amount, 0); // Keep negative for expenses
  ...
  transactions
    ...
    .forEach(t => {
      ...
      categoryMap.set(t.category, existing + Math.abs(t.amount));
    });
  ...
  // Monthly comparison
  const monthlyComparison = engine.getSpendingTrends();
